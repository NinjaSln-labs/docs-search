...
from docs_search_core import (
    win_utf8, resolve_docs_dir, resolve_db_path, ensure_index,
    get_conn, load_meta, sanitize_filename, dedupe_target,
    rebuild_index, MAX_UPLOAD_BYTES,
)
        ...
        def _api_get(self, path, params):
            if path == '/api/stats':
                ...
                try:
                    ...
                    pass
                self._json(200, {'count': n, 'updated': meta.get('updated', '?'),
                ...
                c.close()
                results = [{'path': p, 'cat': c, 'title': t,
                            'snippet': b[:150].replace('\n', ' ')} for p, c, t, b in rows]
                ...
                if not row:
                    ...
                    return
                self._json(200, {'title': row[0], 'body': row[1], 'size': row[2],
                                 'cat': row[3], 'path': p})
            else:
                self._send(404, 'text/plain; charset=utf-8', b'not found')
