...
from docs_search_core import (
    win_utf8, resolve_docs_dir, resolve_db_path,
    load_meta, ensure_index, get_conn, sanitize_filename,
    dedupe_target, MAX_UPLOAD_BYTES, ENV_DOCS_DIR,
)
...
def cmd_index(args):
    ...
    from docs_search_core import rebuild_index
...
def cmd_status(args):
    win_utf8()
    docs_dir, db_path = _paths(args)
    from docs_search_core import need_reindex
...
def cmd_open(args):
    ...
    if sys.platform == 'win32':
        subprocess.Popen(['start', '/B', '', str(p)], shell=True,
...
def main():
    ...
    if not a.cmd:
        p.print_help()
        return
    fn = {'index': cmd_index, 'search': cmd_search, 'list': cmd_list, 'show': cmd_show,
          'status': cmd_status, 'upload': cmd_upload, 'open': cmd_open}.get(ALIAS.get(a.cmd, a.cmd))
    if fn:
        fn(a)
    else:
        p.print_help()
