...
- 远程认证：远端服务启用认证时，`--token`（Bearer）或 `--user/--password`（Basic），
  环境变量 `DOCS_SEARCH_TOKEN` / `DOCS_SEARCH_USER` / `DOCS_SEARCH_PASSWORD` 回退；
  **显式认证参数优先**——显式给了任一认证参数时 env 回退整体关闭（防 env 残留另一路凭据互斥/串凭据）；
  凭据错误探活即失败退出
...
`--mcp-arg` 附加参数，幂等/dry-run），操作手册见 [INSTALL.md](INSTALL.md)。
...
*docs-search v1.3.1 · 协议版本随版本号同步 · [文档站首页](index.html)*
