 ...
 *       代理: DOCS_SEARCH_PROXY=http://ip:port 走指定代理,或 direct/none 强制直连(忽略环境代理);
...
export default function docsSearchExtension(pi: ExtensionAPI) {
	...
	// 远程选项(代理/凭据,远程模式时透传给 server 参数)
	const remoteArgs: string[] = [];
	if (process.env.DOCS_SEARCH_PROXY) remoteArgs.push("--proxy", process.env.DOCS_SEARCH_PROXY);
	if (process.env.DOCS_SEARCH_TOKEN) remoteArgs.push("--token", process.env.DOCS_SEARCH_TOKEN);
	if (process.env.DOCS_SEARCH_USER && process.env.DOCS_SEARCH_PASSWORD) {
		remoteArgs.push("--user", process.env.DOCS_SEARCH_USER, "--password", process.env.DOCS_SEARCH_PASSWORD);
	}
	...
	function ensureServer(): ChildProcess {
		...
		const args = serviceUrl
			? [...extraArgs, ...remoteArgs, "--url", serviceUrl]
